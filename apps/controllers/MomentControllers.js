...
export const getMoment = async (req, res) => {
    try {
        ...
        console.log("error get moment", error);
 ...
 * @returns 
 */
export const getMyAllMoment = async (req, res) => {
    try {
        const usersToken = getDataUserUsingToken(req, res);
        const users_id = usersToken.tod;
...
export const likeMomentPerContentDetail = withTransaction(
...
export const getLikeMomentContentDetail = async (req, res) => {
    try {
...
export const commentMomentPerContentDetail = withTransaction(
    async (req, res, transaction) => {
        try {
...
export const getDetailMomentPerContentDetail = async (req, res) => {
    try {
        const usersToken = getDataUserUsingToken(req, res);
        const users_id = usersToken.tod;
        const slugPostContentDetail = req.params.slugPostContentDetail;
        ...
        if (Number(users_id) > 0) {
            const alreadyImpressed = await ImpressionPostContentDetailModels.findOne({
                where: { users_id, post_content_details_id: getIdPostContentDetail.id },
            });
            if (!alreadyImpressed) {
...
export const createMomentContentDetail = withTransaction(
    async (req, res, transaction) => {
        try {
            const usersToken = getDataUserUsingToken(req, res);
            const users_id = usersToken.tod;
...
export const deleteDetailMomentPerContentDetail = withTransaction(
    async (req, res) => {
        try {
            const usersToken = getDataUserUsingToken(req, res);
            const users_id = usersToken.tod;
...
export const getCommentMomentContentDetail = async (req, res) => {
    try {
        const page = parseInt(req.query.page, 10) || 1;
        const limit = parseInt(req.query.limit, 10) || 10;
        const offset = (page - 1) * limit;
...
export const deleteCommentMoment = withTransaction(
    async (req, res) => {
        try {
